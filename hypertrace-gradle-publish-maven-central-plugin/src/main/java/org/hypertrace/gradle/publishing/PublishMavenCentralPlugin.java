...
public class PublishMavenCentralPlugin implements Plugin<Project> {
  ...
  private void applyNexusPlugin() {
    ...
    project.getRootProject().getPluginManager().apply(NexusPublishPlugin.class);
  }

  private void configureNexusPlugin() {
    Optional<String> user = getProperty(PROPERTY_OSSRH_USERNAME);
    Optional<String> password = getProperty(PROPERTY_OSSRH_PASSWORD);
    getNexusPublishExtension()
        .getRepositories()
        .sonatype(
            nexusRepository -> {
              nexusRepository
                  .getNexusUrl()
                  .set(project.uri("https://s01.oss.sonatype.org/service/local/"));
              if (user.isPresent() && password.isPresent()) {
                nexusRepository.getUsername().set(user.get());
                nexusRepository.getPassword().set(password.get());
              }
            });
    // packageGroup
    getNexusPublishExtension().getPackageGroup().set(this.extension.packageGroup);
  }
                  ...
                  mavenPom -> {
                    ...
                    mavenPom
                        ...
                        .set(
                            project.provider(
                                () ->
                                    String.format("%s:%s", project.getGroup(), project.getName())));
  ...
  private NexusPublishExtension getNexusPublishExtension() {
    return project.getRootProject().getExtensions().getByType(NexusPublishExtension.class);
  }
  ...
  private void validateGradlePropertiesBeforeStagingTasks() {
    project
        .getRootProject()
        .getTasks()
        .withType(AbstractNexusStagingRepositoryTask.class)
        .forEach(
            task ->
                task.doFirst(
                    unused -> {
